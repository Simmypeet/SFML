...
TEST_CASE("[Graphics] sf::StencilMode Draw", runDisplayTests())
{
    ...
    SECTION("Comparisons")
    {
        ...
        SECTION("GreaterEqual")
        {
            renderTexture
                .draw(shape1,
                      sf::StencilMode(sf::StencilComparison::GreaterEqual, sf::StencilUpdateOperation::Keep, 126, 0xFF, false));
            renderTexture.display();
            CHECK(renderTexture.getTexture().copyToImage().getPixel({50, 50}) == sf::Color::Red);
            renderTexture
                .draw(shape1,
                      sf::StencilMode(sf::StencilComparison::GreaterEqual, sf::StencilUpdateOperation::Keep, 127, 0xFF, false));
            renderTexture.display();
            CHECK(renderTexture.getTexture().copyToImage().getPixel({50, 50}) == sf::Color::Green);
            renderTexture
                .draw(shape2,
                      sf::StencilMode(sf::StencilComparison::GreaterEqual, sf::StencilUpdateOperation::Keep, 128, 0xFF, false));
            renderTexture.display();
