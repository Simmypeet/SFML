...
std::shared_ptr<Display> openDisplay()
{
    ...
    auto sharedDisplay = UnixDisplayImpl::display.lock();
    if (!sharedDisplay)
    {
        sharedDisplay.reset(XOpenDisplay(nullptr), XCloseDisplay);
        UnixDisplayImpl::display = sharedDisplay;
...
std::shared_ptr<_XIM> openXim()
{
    ...
    assert(!UnixDisplayImpl::display.expired() &&
    ...
    if (!sharedXIM)
    {
        ...
        sharedXIM.reset(XOpenIM(UnixDisplayImpl::display.lock().get(), nullptr, nullptr, nullptr), XCloseIM);
        xim = sharedXIM;
